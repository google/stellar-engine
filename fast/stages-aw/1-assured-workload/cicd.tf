...
locals {
  ...
  cicd_workflow_providers = {
    bootstrap   = "1-assured-workload-providers.tf"
    bootstrap_r = "1-assured-workload-r-providers.tf"
    resman      = "2-resman-providers.tf"
    resman_r    = "2-resman-r-providers.tf"
    tenant      = "5-tenant-providers.tf"
    tenant_r    = "5-tenant-providers.tf"
  }
  cicd_workflow_var_files = {
    bootstrap = []
    resman = [
      "1-assured-workload.auto.tfvars.json",
      "1-globals.auto.tfvars.json"
    ]
    tenant = []
  }
}
...
module "automation-tf-cicd-sa" {
  ...
  iam_storage_roles = merge(
    {
      (module.automation-tf-output-gcs.name) = ["roles/storage.objectViewer"]
    },
    {
      (lower("${element(split(".", var.top_level_folder.name), 0)}-org-iac"))           = ["roles/storage.objectViewer"]
      (lower("${element(split(".", var.top_level_folder.name), 0)}-org-iac-bootstrap")) = ["roles/storage.objectViewer"]
      ("${var.prefix}-il5-prod-iac-core-config")                                        = ["roles/storage.objectViewer"]
    }
  )
}
