...
resource "google_billing_account_iam_member" "security_billing_admins" {
  billing_account_id = var.billing_account.id
  role               = "roles/billing.admin"
  member             = "serviceAccount:${module.branch-security-sa.email}"
}
...
module "branch-security-gcs" {
  source        = "../../../modules/gcs"
  project_id    = var.automation.project_id
  name          = lower("${replace(lookup(var.regime_mapping, var.assured_workloads.regime, var.assured_workloads.regime), "_", " ")}-prod-resman-sec")
  prefix        = var.prefix
