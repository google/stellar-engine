...
locals {
  ...
  cicd_workflow_attrs = {
    ...
    networking = {
      ...
      tf_providers_files = {
        apply = "3-networking-providers.tf"
        plan  = "3-networking-r-providers.tf"
      }
    ...
    security = {
      ...
      tf_providers_files = {
        apply = "4-security-providers.tf"
        plan  = "4-security-r-providers.tf"
      }
      tf_var_files = local.cicd_workflow_var_files.stage_2
    }
    shared_services = {
      service_accounts = {
        apply = try(module.branch-shared-services-sa-cicd[0].email, null)
        plan  = try(module.branch-shared-services-r-sa-cicd[0].email, null)
      }
      tf_providers_files = {
        apply = "shared-services-providers.tf"
        plan  = "shared-services-r-providers.tf"
      }
      tf_var_files = local.cicd_workflow_var_files.stage_2
    }
  }
  ...
  providers = merge(
    {
      "3-networking" = templatefile(local._tpl_providers, {
        backend_extra = null
        bucket        = module.branch-network-gcs.name
        name          = "networking"
        sa            = module.branch-network-sa.email
      })
      "3-networking-r" = templatefile(local._tpl_providers, {
        backend_extra = null
        bucket        = module.branch-network-gcs.name
        name          = "networking"
        sa            = module.branch-network-r-sa.email
      })
      "3-vpn" = templatefile(local._tpl_providers, {
        backend_extra = "prefix = \"vpn\""
        bucket        = module.branch-network-gcs.name
        name          = "networking-vpn"
        sa            = module.branch-network-sa.email
      })
      "4-security" = templatefile(local._tpl_providers, {
        backend_extra = null
        bucket        = module.branch-security-gcs.name
        name          = "security"
        sa            = module.branch-security-sa.email
      })
      "4-security-r" = templatefile(local._tpl_providers, {
        backend_extra = null
        bucket        = module.branch-security-gcs.name
        name          = "security"
        sa            = module.branch-security-r-sa.email
      })
      "shared-services" = templatefile(local._tpl_providers, {
        backend_extra = null
        bucket        = module.branch-shared-services-gcs.name
        name          = "shared-services"
        sa            = module.branch-shared-services-sa.email
      })
      "shared-services-r" = templatefile(local._tpl_providers, {
        backend_extra = null
        bucket        = module.branch-shared-services-gcs.name
        name          = "shared-services"
        sa            = module.branch-shared-services-r-sa.email
      })
    },
  ...
  tenant_resman_providers = templatefile(local._tpl_providers, {
    bucket        = var.automation.tenant_bucket
    backend_extra = null
    name          = "tenants"
    sa            = var.automation.service_accounts.tenant
  })
  service_accounts = merge(
    {
      ...
      shared-services        = module.branch-shared-services-sa.email
      shared-services-r      = module.branch-shared-services-r-sa.email
  ...
  tfvars = {
    ...
    # tenant_folder_ids = {
    #   for k, v in local.tenant_envs : k => module.tenant-top-folders[k].id
    # }
  }
...
output "providers" {
  # tfdoc:output:consumers 03-networking 04-security 03-dataplatform xx-sandbox xx-teams shared-services
...
output "security" {
  # tfdoc:output:consumers 04-security
  description = "Data for the networking stage."
  value = {
...
output "shared_services" {
  description = "Data for the shared-services stage."
  value = {
    gcs_bucket      = module.branch-shared-services-gcs.name
    service_account = module.branch-shared-services-sa.iam_email
  }
}
