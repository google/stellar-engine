...
# Full-Stack GCP & Azure Managed HA VPN Example

This example demonstrates how to provision both the Google Cloud side and the Azure side of an HA VPN connection using the `gcp-azure-vpn` module.

It connects to the Azure API, dynamically reads the Public IPs of an existing Azure Virtual Network Gateway, and automatically builds the corresponding `Local Network Gateways` and `IPsec Connections` in Azure to map back to the newly created GCP HA VPN Gateway.
...
### <a name="module_gcp_azure_vpn_both"></a> [gcp\_azure\_vpn\_both](#module\_gcp\_azure\_vpn\_both)
