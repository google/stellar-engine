...
| Attribute | Detail |
| :--- | :--- |
| **Runbook ID** | IR-CUSTOM-001 |
| **Last Updated** | {{ DATE }} |
| **Owner** | Incident Response Team (ISSO / SOC) |
| **Target SLA** | Triage: 15m \| Containment: 30m \| Resolution: 4h |
...
* **Audit & Log Inspection:** [List roles, e.g., `roles/logging.viewer`, `roles/logging.privateLogViewer` on the {{ ORGANIZATION }} Organization]
* **Security & Posture Management (where SCC is enabled):** [List roles, e.g., `roles/securitycenter.viewer` or `roles/securitycenter.admin`]
* [List specific containment/break-glass roles, e.g., `roles/compute.securityAdmin`, `roles/iam.securityAdmin`]
...
* **[Source/Tool Name, e.g., Security Command Center (where enabled)]**: [Description of finding class, alert ID, or rule name].
* **[Source/Tool Name, e.g., External CSSP / SIEM (where configured)]**: [Description of indicator from Cloud Logging audit sinks].
* **[Source/Tool Name, e.g., Cloud Logging]**: [Description of indicator, specific log sub-type, or automated alert metric].
...
* [Define standard internal escalation triggers and paging procedures for Incident Commander].
* **Military Service Branch & DoD CSSP Escalation Procedures (CJCSM 6510.01B):**
